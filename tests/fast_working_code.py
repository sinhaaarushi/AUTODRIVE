...
while cv2.waitKey(10) != ord('q'):
    ...
    # Resize image for steering angle prediction
    image = cv2.resize(full_image[-150:], (200, 66)) / 255.0

    # Use ThreadPoolExecutor to run predictions concurrently
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_steering = executor.submit(predict_steering_angle, image)
        future_segmentation = executor.submit(process_segmentation, full_image, lane_model, object_model)
